...
| `modules/agents/` | **spec 006** — the intelligence layer's compute. Bedrock agents (classic) for the digest and suggester with their action-group Lambdas, aliases, and one shared guardrail; a worker Lambda per capability; EventBridge Scheduler daily triggers ordered scan (06:00) → metrics (07:30) → advisor (08:00) → digest (09:00) → suggester (10:00). The advisor deploys a worker and a schedule and **no agent resources** — its run is deterministic (tasks.md T038f). The metrics collector carries `cloudwatch:GetMetricData` and the same ExternalId/scanner-role scope every account-touching worker has; it is the one P2 cost that grows with inventory, and the module states the dev posture (R-606). **The Bedrock-invoking workers cannot reach Bedrock from inside the VPC** until R-407 is funded (R-605); `modules/network/` gates the Bedrock and API Gateway interface endpoints behind `enable_agent_endpoints` for verification windows. Bedrock Agents (classic) is in maintenance mode and this account cannot create one — see constitution v3.0.0 and spec 006 R-613; migration to AgentCore Runtime is T061–T067 |
