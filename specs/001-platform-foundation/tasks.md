...
- [X] T137 Sweep every project-authored `.md` file for staleness against the current state (constitution version, task counts, authorizer type, deploy patterns). Found and fixed two: `ops/runbooks/provisioning.md` pinned a constitution version as a live rule rather than a historical citation, and `quickstart.md` still told a reader to use the `/dev/stdout` invoke anti-pattern (the exact bug fixed elsewhere) and gave no instructions for obtaining `$TOKEN` or the ID-token requirement it needs — Principle I
- [X] T138 Amend the `pr-task-reference` gate (T135) to accept a spec-ID reference (e.g. `002-account-onboarding-and-discovery`) in place of a task ID, but only for a PR whose changed files are entirely under `specs/`. Found live opening spec 2's own first PR: a spec's `/speckit-specify`/`/speckit-clarify`/`/speckit-plan` PRs necessarily precede `tasks.md`'s existence, so the original task-ID-only gate would have blocked every future spec's first PR. Any PR touching code or infra outside `specs/` still requires a real `T\d{3}` task ID — the spec-ID exemption does not weaken F2's original fix — Principle I, Principle VII
