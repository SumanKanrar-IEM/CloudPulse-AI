...
**Phase 5a (P1 work, deferred)**: T032–T038 — the AgentCore migration forced by AWS closing
Bedrock Agents (constitution v3.0.0, R-613). Scheduled after P2 because P2 does not depend on which
runtime hosts an agent, and T032's spike could still change the shape of T033–T038.

**P2 (stretch)**: Phases 6–10, T039–T060 (renumbered by Phase 5a's insertion). Every P2 task is marked **[P2]** in its description.
...
## Phase 5a: AgentCore Migration (Priority: P1, deferred — runs after P2)
...
does not depend on it, and T032's outcome could still change the shape of T033–T038.
...
- [ ] T032 **Spike, before anything is rewritten.** Deploy a minimal agent to AgentCore Runtime in
      ...
      somebody once recorded a capability claim nobody had exercised. **No T033–T038 work starts
...
- [ ] T033 [P] Rewrite `agents/definitions/{digest,suggester}.json` for AgentCore's definition
      format; keep R-608's content-hash contract intact — S43, S44, FR-005, R-608, R-613
- [ ] T034 Replace `connectors/aws.py::invoke_agent` with its AgentCore equivalent — still the only
      ...
      genuinely benefit.
- [ ] T035 Adapt `agents/action-groups/_platform_api.py` from the Bedrock Agents event envelope to
...
- [ ] T036 Rewrite `infra/modules/agents/` for AgentCore: runtime, its execution role, guardrail
...
- [ ] T037 [P] Re-run the full suite unchanged and confirm it still passes. **This is the
...
- [ ] T038 Live-verify the migrated layer and tear down immediately after, per playbook §0.5.3 and
