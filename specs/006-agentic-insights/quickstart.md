...
> **The governance data these scenarios need cannot be produced live either — read this before
> planning a session around them.** The bullet above asks for a tenant with findings and spend.
> Findings come from scanned resources; scanning needs a registered account; and
> `register_account` calls STS and the Resource Groups Tagging API, which have no route out of a
> VPC with no NAT gateway. Spec 005's T051a established this empirically after the same mistake
> was made one artifact over: R-511 reasoned correctly about utilization making no AWS call and
> missed that its *input* could not be produced live.
>
> So the blocked precondition is **two-deep**, not one: the model may be unreachable *and* the
> data the model would summarise cannot be generated by a live scan. **V1, V2, V3, V4, V6 and V7
> all depend on scanned inventory; V5 depends on metrics, which depend on the same scan.** Every
> scenario in this file therefore runs against **seeded fixture data**, not a live-scanned
> account, until R-407 is funded — and that is a property of the environment, not a defect in
> these scenarios.
>
> What a live session *can* still prove: the deploy is healthy, the routes exist and are
> role-gated, the workers and schedules have the intended shape, and the degraded paths behave as
> FR-007a and SC-009 specify. That is the honest bound. State it before the session, not during.
