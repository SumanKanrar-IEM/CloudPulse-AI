...
- [X] T057 [P] **[P2]** Write `backend/tests/integration/test_scan_history.py` — trigger, timing, counts, outcome retrievable per scan — S17, FR-033. **Moved from `tests/unit/` to `tests/integration/`**, the same class of deviation tasks.md already documents for T039/T052: proving real trigger/timing/counts/ordering needs actual persisted rows through a real PostgreSQL container, not a mocked session — there is no lighter-weight fixture already established in this codebase that would prove the same thing honestly.
- [X] T058 **[P2]** Write `GET /accounts/{id}/scans` (scan history), any-role-readable like the accounts view — S17, FR-033. Reuses the `Scan` response model `trigger_scan` (T048) already defined; added `ScansList`, a `_to_scan_model` row mapper, and the route itself. Imports the ORM `Scan` model aliased as `ScanRow` to avoid colliding with the existing pydantic `Scan` response model in the same file.
