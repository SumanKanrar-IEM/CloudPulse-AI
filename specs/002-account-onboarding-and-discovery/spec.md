...
successfully. Adding an account, and seeing why one has stopped working, both happen here — never
by going into the AWS console except to deploy the cross-account template itself.
...
- **FR-010**: The platform MUST expose a view listing every registered account together with its
  connection mode, region list, current verification status, and most recent scan outcome.
- **FR-011**: An operator MUST be able to register a new account, and trigger an on-demand scan of
  an existing one, from this view without leaving it.
...
- **Account**: a registered AWS account belonging to a tenant, holding its connection mode,
  role reference, and scan-region list — never a credential (spec 1's schema; behavior specified
  here).
...
- **The cross-account template is a standard AWS-native deployment artifact** an operator applies
  ...
  ever connected.
- **Deregistering or deactivating an account is out of scope for this spec's P1 path.** An
  operator can add accounts and edit their region lists; removing an account entirely (and
  deciding whether its historical resource and scan data is purged or retained read-only) is
  deferred — spec 1's precedent (audit events retained indefinitely, nothing silently purged)
  is the likely direction, but is not decided here to avoid scope creep into spec 3's findings
  lifecycle, which would also need to define what happens to findings against a deregistered
  account's resources.
...
- Deregistering or deactivating a connected account (see Assumptions).
