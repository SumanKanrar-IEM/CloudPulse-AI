...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("connection_mode", postgresql.ENUM(name="connection_mode", create_type=False), nullable=False),
        ...
        sa.Column("scan_regions", postgresql.ARRAY(sa.Text()), server_default=sa.text("'{}'::text[]"), nullable=False),
        sa.Column("status", postgresql.ENUM(name="account_status", create_type=False), server_default="pending", nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.PrimaryKeyConstraint("id", name="pk_cloud_account"),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenant.id"], name="fk_cloud_account_tenant_id_tenant", ondelete="RESTRICT"),
        sa.UniqueConstraint("tenant_id", "aws_account_id", name="uq_cloud_account_tenant_account"),
        sa.CheckConstraint("aws_account_id ~ '^[0-9]{12}$'", name="ck_cloud_account_aws_account_id_is_12_digits"),
        sa.CheckConstraint("connection_mode <> 'assume_role' OR role_arn IS NOT NULL", name="ck_cloud_account_assume_role_requires_arn"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenant.id"], name="fk_resource_tenant_id_tenant", ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["cloud_account_id"], ["cloud_account.id"], name="fk_resource_cloud_account_id_cloud_account", ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["parent_resource_id"], ["resource.id"], name="fk_resource_parent_resource_id_resource", ondelete="SET NULL"),
