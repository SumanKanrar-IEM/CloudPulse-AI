...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenant.id"], name="fk_rule_tenant_id_tenant", ondelete="RESTRICT"),
    ...
    op.create_table(
        ...
        sa.Column("status", postgresql.ENUM(name="finding_status", create_type=False), server_default="open", nullable=False),
        sa.Column("opened_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.Column("resolved_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.PrimaryKeyConstraint("id", name="pk_finding"),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenant.id"], name="fk_finding_tenant_id_tenant", ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["resource_id"], ["resource.id"], name="fk_finding_resource_id_resource", ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["rule_id"], ["rule.id"], name="fk_finding_rule_id_rule", ondelete="RESTRICT"),
        sa.CheckConstraint("status <> 'resolved' OR resolved_at IS NOT NULL", name="ck_finding_resolved_requires_timestamp"),
    )
    op.create_index("ix_finding_tenant_id", "finding", ["tenant_id"])
    op.create_index("ix_finding_tenant_status_severity", "finding", ["tenant_id", "status", "severity"])
