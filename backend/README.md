...
| Path | Owner | Contents |
|---|---|---|
| `app/api/` | spec 001 (extended by 002–004) | FastAPI app, error envelope, correlation middleware, routers. `app/api/routers/resources.py` (**spec 004**) is the governance dashboard's inventory read surface (`GET /resources`, `GET /resources/{id}`) — paged/filtered server-side, never the full inventory into a client at once (FR-011) |
